...
def dump_feature_2D (feature,filename,max_range,min_range,png_jpg_flag):
    ...
    if png_jpg_flag==0:
        scipy.misc.imsave(filename_write, feature_2D) #,cmap=matplotlib.cm.gray
    else:
    ...
    print (str(os.path.getsize(filename_write)))
    ...
    return (torch.Tensor(read_3D_feature)).cuda()
...
def Quantize_255(center_feature, n_bits=8):  # I consider numbers to be 255
    ...
    # print "range_center_inv_bit_range is {}".format(range_center_inv_bit_range)
    # range_center_inv_bit_range    = Variable(range_center_inv_bit_range, requires_grad=True)
    # print "V_minus_min_v is {}".format(V_minus_min_v)
    value_before_round = V_minus_min_v * range_center_inv_bit_range.expand_as(V_minus_min_v)
    # print value_before_round
    # value_after_round = value_before_round.round()
    # quantized_value_255 = value_after_round

    # print value_after_round
...
def Quantize_center_VALIDATION(center_feature, n_bits=8):  # I consider numbers to be 255
    ...
    if torch.eq(min_center_value, max_center_value).all():
        max_center_value = max_center_value.add(10 ^ (-20))
    V_minus_min_v = center_feature - min_center_value.expand_as(center_feature)
    range_center = max_center_value.add(-1 * min_center_value)
    # print range_center
    range_center_inv = range_center.pow(-1)
