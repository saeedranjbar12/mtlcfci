...
class Compose_depth(object):
    def __init__(self, augmentations):
        self.augmentations = augmentations

    def __call__(self, img, mask,depth):
        img, mask, depth = Image.fromarray(img, mode='RGB'), Image.fromarray(mask, mode='L') , Image.fromarray(depth, mode='I')             
        assert (img.size == mask.size and img.size == depth.size)
        for a in self.augmentations:
            img, mask, depth = a(img, mask,depth)
        return np.array(img), np.array(mask, dtype=np.uint8), np.array(depth, dtype=np.int32)

class RandomRotate_depth(object):
    def __init__(self, degree):
        self.degree = degree
...
class RandomHorizontallyFlip_depth(object):
    def __call__(self, img, mask,depth):
        if random.random() < 0.5:
